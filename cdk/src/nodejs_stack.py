...
class NodejsService(Stack):
    @property
    def mesh_nodejs_virtual_service_arn(self):
        return self.mesh_nodejs_vs.virtual_service_arn

    @property
    def mesh_nodejs_virtual_service_name(self):
        return self.mesh_nodejs_vs.virtual_service_name
    ...
    def __init__(
        ...
        services_security_group: aws_ec2.SecurityGroup,
        service_discovery_namespace: aws_servicediscovery.PrivateDnsNamespace,
        # mesh_arn: str,
        ...
        **kwargs
        ...
        log_group = aws_logs.LogGroup(
            self, "ecsworkshopNodejs", retention=aws_logs.RetentionDays.ONE_WEEK
        )
        ...
        fargate_service = aws_ecs.FargateService(
            ...
            cluster=ecs_cluster,
            security_groups=[services_security_group],
            desired_count=1,
        ...
        fargate_task_def.add_to_task_role_policy(
            aws_iam.PolicyStatement(actions=["ec2:DescribeSubnets"], resources=["*"])
        )
        ...
        # self.appmesh(mesh_arn, mesh_virtual_gateway_name)

    def appmesh(self, mesh_arn: str, mesh_virtual_gateway_name: str):
        ...
        mesh_vgw = aws_appmesh.VirtualGateway.from_virtual_gateway_attributes(
            ...
            "Mesh-VGW",
            mesh=mesh,
            virtual_gateway_name=mesh_virtual_gateway_name,
        )
        ...
        envoy_container = self.fargate_task_def.add_container(
            ...
            essential=True,
            logging=aws_ecs.LogDriver.aws_logs(
                stream_prefix="/mesh-envoy-container", log_group=self.log_group
            ),
        ...
        self.container.add_container_dependencies(
        ...
        self.mesh_nodejs_vs = aws_appmesh.VirtualService(
            self,
            "mesh-nodejs-vs",
